...
- (void)getDataCompletionHandle:(void (^)(NSError *))completed {
    ...
    self.dataTask = [LoadMoreNetManager getCategoryForCategoryId:_categoryId tagName:_name pageSize:_pageSize completionHandle:^(id responseObject, NSError *error) {
        self.model = (ContentCategoryModel *)responseObject;
        NSLog(@"转换成model为%@",self.model);
        completed(error);
    }];
}
