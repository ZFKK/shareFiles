...
+(id)GET:(NSString *)path parameters:(NSDictionary *)dictionary complicationHandler:(void (^)(id, NSError *))complicated
{
    //NSLog(@"Request Path: %@, params %@", path, params);
    //NSString *urlStr = [path stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    return [[self defaultManager] GET:path parameters:dictionary progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        complicated(responseObject,nil);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        NSLog(@"GET ERROR: %@",error.userInfo);
    }];
}

+(id)POST:(NSString *)path parameters:(NSDictionary *)dictionary complicationHandler:(void (^)(id, NSError *))complicated{
    return [[self defaultManager] POST:path parameters:dictionary progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        complicated(responseObject,nil);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
