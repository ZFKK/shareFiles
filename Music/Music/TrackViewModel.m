...
-(instancetype)initWithitemModel:(NSInteger)itemtype{
    if (self == [super init]){
        _itemtype = itemtype;
    }
    return self;
...
-(void)getDataCompletionHandle:(void (^)(NSError *))completed{
    self.dataTask = [LoadMoreNetManager getTracksForAlbumId:_albumId mainTitle:_title idAsc:_asc completionHandle:^(id responseObject, NSError *error) {
        DestinationModel *model = (DestinationModel *)responseObject;
        self.model = model;
        completed(error);
    }];
}
#pragma mark -关于播放记录的，感觉想法还是可以的
-(void)getItemModelData:(void (^)(NSError *))completed{
    if (_itemtype == HistoryModel){//不知道这里为什么没有提示
        NSArray *managerArray = [[PlayMusicManager sharedInstance] historyMusicItems];
        //取出管理者所存放的历史记录数组
        ...
        //把这个字典模型重新转换成一个新的模型数据
    }
    
    //对于最爱的，和上边的类似
    if (_itemtype == FavoriteModel) {
        NSArray *managerArray = [[PlayMusicManager sharedInstance] favoriteMusicItems];
        NSDictionary *listDictionary = [[NSDictionary alloc] initWithObjects:@[managerArray] forKeys:@[@"list"]];
        NSDictionary *tracksDictionary = [[NSDictionary alloc] initWithObjects:@[listDictionary] forKeys:@[@"tracks"]];
        _model = [DestinationModel mj_objectWithKeyValues:tracksDictionary];
    }
}
#pragma mark -获取各个参数的方法
