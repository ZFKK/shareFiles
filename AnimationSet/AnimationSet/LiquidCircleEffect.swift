...
class LiquidCircleEffect : LiquidLoadEffect {
    ...
    internal override init(loader: LiquidLoad, color: NSColor) {
    ...
    override func willSetup() {
        ...
        self.engine = SimpleEngine(radiusThresh: self.circleRadius * 0.85, angleThresh: 0.5)
        let moveCircleRadius = circleRadius * moveScale
        moveCircle = LiquidTableCircle(center: movePosition(0.0), radius: moveCircleRadius, color: self.color)
    }
